...
model = dict(
    ...
    decode_head=dict(
        ...
        act_cfg=dict(type='ReLU', inplace=True),
        align_corners=True,
        ignore_index=255,  # Set at head level
        loss_decode=[
            dict(
                type='CrossEntropyLoss',
                use_sigmoid=False,
                loss_weight=0.4),  # Removed class_weight to avoid index issues
            dict(
...
# ========== Data Loaders ==========
# IMPORTANT: Check if ACDC needs reduce_zero_label
# ACDC uses Cityscapes format. If labels are 0-18 with 255 ignore, reduce_zero_label=False
# If labels are 1-19 with 0=background/ignore, you need reduce_zero_label=True
train_dataloader = dict(
    ...
    dataset=dict(
        ...
        # reduce_zero_label=False,  # Uncomment and set to True if labels are 1-19
